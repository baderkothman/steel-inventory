...
use crate::{
    models::{
        InvoiceDetail, InvoiceFilters, InvoiceListRow, InvoiceSaveResult, PurchaseInvoicePayload,
    },
...
#[tauri::command]
pub fn restore_purchase_invoice(state: State<'_, AppState>, id: i64) -> Result<(), AppError> {
    let user_id = state.require_user_id()?;
    let conn = state.open_conn()?;
    purchase_service::restore_purchase_invoice(&conn, user_id, id)
}
...
pub fn permanently_delete_purchase_invoice(
    ...
    let user_id = state.require_user_id()?;
    let conn = state.open_conn()?;
    purchase_service::permanently_delete_purchase_invoice(&conn, user_id, id)
}
